...
The current source version is `0.1.1`. It is suitable for evaluation and feedback, not a stable release.
...
Download the Apple Silicon DMG from the [`v0.1.1` Release](https://github.com/Goldloli/asrbox/releases/tag/v0.1.1) and verify `SHA256SUMS.txt`. The package is unsigned and unnotarized, so first launch requires right-clicking the app and choosing Open, or allowing it under System Settings → Privacy & Security.
