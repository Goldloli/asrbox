...
An in-app download requires both the platform-matching installer (Apple Silicon DMG or Windows x64 NSIS executable) and `SHA256SUMS.txt` in the same official Release. Each checksum entry must name the installer by its exact Release asset basename, such as `ASRbox_<version>_aarch64.dmg` or `ASRbox_<version>_x64-setup.exe`; a leading `./` or any other path component is intentionally not accepted. The download is first written as a `.part` file in the system Downloads directory, then renamed only after SHA-256 verification. Cancelling, a missing checksum, a mismatched checksum, an unexpected asset URL, or a renamed release asset prevents the file from being opened; use the browser Releases fallback and verify the asset manually instead of bypassing the error.

If the error says `SHA256SUMS.txt does not contain <installer>`, the Release's manifest does not match the updater's exact-basename contract (v0.2.1 shipped such a manifest). Already-published Releases are left untouched: their installers remain valid when verified manually in a browser, and in-app updates resume as soon as a newer Release with a canonical basename manifest (v0.3.0 or later) becomes the latest Release — the updater always resolves the channel's latest Release, so no repair of older Release assets is needed or performed. Future Releases are generated from one final manifest and fail the release gate if either maintained installer cannot be resolved exactly.
