...
If the error says `SHA256SUMS.txt does not contain <installer>`, compare the installer name with the manifest entry character for character. An affected already-published Release must be repaired by replacing only its checksum manifest with canonical basename entries while preserving and re-verifying every digest. A later application-code fix cannot retroactively change an already-installed client's parser. After the manifest is corrected, the existing client can retry the same download; future Releases are generated from one final manifest and fail the release gate if either maintained installer cannot be resolved exactly.
