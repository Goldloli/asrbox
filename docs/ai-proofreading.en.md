...
Batches contain at most 100 target segments and 6,000 Unicode characters for remote protocols; local Ollama uses at most 16 segments / 1,600 characters. Character counts include context: at most two neighboring segments on each side, totaling at most 1,000 characters. A segment over 6,000 characters is rejected before any request. Sources require unique segment IDs, nonempty text and exportable timestamps. These character limits are not token guarantees; when a batch still hits context or output capacity limits, it is halved and retried deterministically (recursively down to a single segment), and the run fails only if a single segment still fails. Beyond structure, results are checked for content alignment (neighbouring translations overlapping far beyond their sources, one translation rendered at unrelated segments, filler or ballooned output): a batch that fails is retried down the same split path, and once every batch is done the assembled result is checked again - flagged segments are retranslated once on local Ollama, and anything still failing ends the run with `TRANSLATION_ALIGNMENT_UNVERIFIED`, publishing no translation version while keeping successful batches resumable. Protocols that may charge per request (anything other than local Ollama) never issue that repair on their own: the run fails first, and the flagged segments are retranslated only after you explicitly resume it.
