...
- [x] 3.2 不改动已发布的 v0.2.1 资产（替换已发布 Release 资产属高风险操作且更新器只针对频道最新 Release）；改由发布流水线已修复的 v0.3.0 承担恢复：publish job 汇总资产生成纯 basename 的 `SHA256SUMS.txt`，使 v0.2.0 客户端在检查更新时命中 v0.3.0 并能按精确 basename 解析校验和，恢复升级路径。
