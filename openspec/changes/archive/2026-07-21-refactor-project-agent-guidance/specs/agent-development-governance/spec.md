...
The repository SHALL provide a root `AGENTS.md` that contains ASRbox-specific development rules and routes agents to focused repository sources of truth instead of duplicating full product documentation.

#### Scenario: Agent starts work in ASRbox
- **WHEN** an agent begins a development task in the repository
- **THEN** it can identify the architecture, stability, privacy, CI, and release documents that govern the affected area
