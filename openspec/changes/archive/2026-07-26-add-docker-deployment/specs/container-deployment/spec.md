...
The supported Compose configuration SHALL bind the published port to host loopback by default and SHALL allow the bind address, host port, and optional API token to be configured explicitly.

#### Scenario: Default Compose deployment starts
- **WHEN** an operator starts Compose without overriding network variables
- **THEN** ASRbox is reachable from the Docker host at `127.0.0.1:17494` and is not published on every host interface

#### Scenario: Operator enables LAN access
- **WHEN** an operator explicitly selects a non-loopback bind address
- **THEN** the documentation identifies the API token, trusted-network, VPN, and TLS reverse-proxy precautions and does not describe ASRbox as an Internet-facing security gateway
