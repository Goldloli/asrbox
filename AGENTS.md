...
每次新会话在编辑前，必须先运行 `git status --short --branch` 和 `git fetch --prune origin`，再比较当前 `HEAD` 与其上游分支；不得仅凭可能过期的本地 `origin/main` 引用判断代码已是最新。

- 当前分支落后且工作树干净时，只使用 fast-forward 方式同步（如 `git pull --ff-only`），同步完成后再开始修改。
- 工作树已有修改时，先识别修改归属并以可恢复方式保护它们，再同步和重新应用；禁止为拉取新代码而覆盖修改、执行破坏性 reset，或未经检查直接解决冲突。
- 无法访问远端或没有上游分支时，应明确报告“未验证是否为最新代码”，不得声称已经同步。
...
- 任务中心字幕编辑只经"编辑字幕"模式（只读/编辑共用同一行布局）与并列的"一键替换"页签，保存走既有 `PUT /tasks/{id}/segments` 批量提交并创建恰好一个不可变 `edit` 版本；不得恢复逐段时间/说话人表单或自由全文编辑工具。
